...
def dbattack(fuzz_val, top_k_words, qrs, sample_index, hypotheses,random_text_, premise, true_label,orig_label,
           predictor, stop_words_set, word2idx, idx2word, cos_sim, sim_predictor=None,
           import_score_threshold=-1., sim_score_threshold=0.5, sim_score_window=15, synonym_num=50,
           batch_size=32, embed_func=''):
    ...
    for t in range(100):
            ...
            for i in range(len(text_ls)):
                if x_t[i] != text_ls[i]:
                    ...
                    if np.sum(pr) > 0:
                        ...
                        if semantic_sims>=best_sim:
                            best_attack = new_text[:]
                            best_sim = float(semantic_sims)
        ...
        p = torch.softmax(torch.tensor(x_t_perturb_dist[nonzero_ele]), dim=0)
        perturb_word_idx_list = []
        while len(perturb_word_idx_list) < len(nonzero_ele):
            idx = np.random.choice(nonzero_ele, p=p)
        ...
        for perturb_word_idx in perturb_word_idx_list:
            ...
            while len(n_samples) < n_sample:
                ...
                if syn_idx not in n_samples and syn_idx!=ori_idx:
        ...
        if np.sum(pr) > 0:
            ...
            if (sim_new >= best_sim or False) and (
